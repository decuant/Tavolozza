...
--	m_logger:line("ReadSettings")
	...
	if tSettings then m_Sideframe.tWinProps = tSettings end
end
...
--	m_logger:line("SaveSettings")
	...
	local tWinProps = m_Sideframe.tWinProps
	local sLine
...
--
local function OnCloseSideframe()
--	m_logger:line("OnCloseSideframe")
  
	if not m_Sideframe.hWindow then return end

	-- need to convert from size to pos
	--
	local pos  = m_Sideframe.hWindow:GetPosition()
	local size = m_Sideframe.hWindow:GetSize()
	...
	tWinProps.use_font	= m_Sideframe.tWinProps.use_font				-- just copy over
	
	m_Sideframe.tWinProps = tWinProps			-- switch structures

	SaveSettings()								-- write to file

	m_Sideframe.hWindow.Destroy(m_Sideframe.hWindow)
	m_Sideframe.hWindow = nil
end
...
--	m_trace:line("OnSetColour")	
	...
	m_Sideframe.hWindow:Refresh()
end

-- ----------------------------------------------------------------------------
-- create a window
--
local function OnCreateSideframe()
--	m_trace:line("OnCreateSideframe")
...
--	frame:Connect(wx.wxEVT_SIZE,		 OnSizeSideframe)
	frame:Connect(wx.wxEVT_CLOSE_WINDOW, OnCloseSideframe)
	...
	m_Sideframe.hWindow	 = frame
	m_Sideframe.hFunCb	 = hFunCb
	m_Sideframe.hOptCb	 = hOptCb
	m_Sideframe.tTargets = tTargets
...
local function SetupPublic()
	...
	m_Sideframe.CreateSideframe	= OnCreateSideframe
	m_Sideframe.CloseSideframe	= OnCloseSideframe
	m_Sideframe.SetColour		= OnSetColour
	m_Sideframe.SetIndex		= OnSetIndex
end
